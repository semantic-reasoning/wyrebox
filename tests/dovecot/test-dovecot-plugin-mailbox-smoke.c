...
typedef struct
{
  ...
  char *response_request_id_override;
} FakeServer;
...
fake_server_thread_main (gpointer user_data)
{
  ...
  if (server->response_request_id_override != NULL) {
    g_clear_pointer (&request_id, g_free);
    request_id = g_strdup (server->response_request_id_override);
  }
...
fake_server_start (FakeServer *server, const char *socket_path,
    FakeServerBehavior behavior, const char *response_request_id_override)
{
  g_autoptr (GError) error = NULL;
  ...
  server->response_request_id_override =
      g_strdup (response_request_id_override);
...
fake_server_join (FakeServer *server)
{
  ...
  g_clear_pointer (&server->response_request_id_override, g_free);
}
#endif
...
test_open_and_get_status_after_open (void)
{
  ...
  fake_server_start (&server, socket_path_local,
      FAKE_SERVER_MAILBOX_SELECT_RESPONSE, NULL);
...
test_open_fails_with_daemon_error (void)
{
  ...
  g_autofree char *socket_path_local = make_socket_path (&socket_root);
  fake_server_start (&server, socket_path_local,
      FAKE_SERVER_DAEMON_ERROR_RESPONSE, NULL);
  socket_path = g_steal_pointer (&socket_path_local);
