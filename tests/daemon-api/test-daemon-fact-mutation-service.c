...
static void
append_fact_journal_record_for_view (WyreboxJournalWriter *writer,
    WyreboxDaemonFactMutationKind mutation_kind,
    ...
    guint64 *out_sequence)
{
  ...
  g_assert_true (wyrebox_daemon_fact_mutation_request_init (&mutation,
          mutation_kind,
          "project_keyword", scope_id,
          (const char *[]) { message_id, view_id, NULL }, &error));
  g_assert_no_error (error);
  g_assert_true (wyrebox_daemon_fact_mutation_request_append_journal
      (&mutation, writer, &journal_offset, out_sequence, &error));
  g_assert_no_error (error);
...
static void
test_fact_mutation_service_materializes_registered_wirelog_views (void)
{
  const WyreboxJournalEventType expected[] = {
    WYREBOX_JOURNAL_EVENT_FACT_INSERTED,
    WYREBOX_JOURNAL_EVENT_FACT_INSERTED,
  ...
  append_fact_journal_record_for_view (writer,
      WYREBOX_DAEMON_FACT_MUTATION_INSERT,
      "account-1", "mail-2", "view-important", &sequence);
  ...
  g_assert_cmpuint (query_catalog_uint64 (catalog_path,
          ...
          "WHERE account_id = 'account-1' "
          "AND view_id = 'view-projects';"), ==, 1);
  g_assert_cmpuint (query_catalog_uint64 (catalog_path,
          "SELECT COUNT(*) FROM derived_views "
          "WHERE account_id = 'account-1' "
          "AND view_id = 'view-important';"), ==, 1);
  g_assert_cmpuint (query_catalog_uint64 (catalog_path,
          "SELECT COUNT(*) FROM derived_view_memberships "
          "WHERE account_id = 'account-1' "
          "AND view_id = 'view-projects' "
  ...
  g_assert_cmpuint (query_catalog_uint64 (catalog_path,
          ...
          "AND message_id = 'mail-2' AND is_visible = TRUE;"), ==, 1);
...
test_derived_view_catalog_rejects_invalid_definition (void)
{
  ...
  g_assert_false
      (wyrebox_daemon_fact_mutation_service_register_wirelog_derived_view
      (service, "", "Important", "wirelog:important",
          ".decl project_keyword(message_id: symbol, view_id: symbol)\n"
          ".decl show_in_virtual_folder(view_id: symbol, message_id: symbol)\n"
          "show_in_virtual_folder(\"view-important\", message_id) :- "
          "project_keyword(message_id, \"view-important\").\n",
  ...
  g_assert_false
      (wyrebox_daemon_fact_mutation_service_register_wirelog_derived_view
      (service, "view-important", "", "wirelog:important",
          ".decl project_keyword(message_id: symbol, view_id: symbol)\n"
          ".decl show_in_virtual_folder(view_id: symbol, message_id: symbol)\n"
          "show_in_virtual_folder(\"view-important\", message_id) :- "
          "project_keyword(message_id, \"view-important\").\n",
...
main (int argc, char **argv)
{
  ...
  g_test_add_func ("/daemon-api/fact-mutation-service/"
      "derived-view-catalog-rejects-invalid-definition",
      test_derived_view_catalog_rejects_invalid_definition);
  g_test_add_func ("/daemon-api/fact-mutation-service/"
