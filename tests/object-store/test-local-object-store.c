...
test_duplicate_put_is_idempotent_and_does_not_rewrite(void)
{
  ...
  g_assert_true(wyrebox_local_object_store_put_bytes(store, input, &first_key, &error));
  g_assert_no_error(error);
  path = object_path_for_key(root, first_key);
  g_assert_cmpint(g_stat(path, &before), ==, 0);

  g_assert_true(wyrebox_local_object_store_put_bytes(store, input, &second_key, &error));
  g_assert_no_error(error);
...
test_different_bytes_produce_different_keys(void)
{
  ...
  g_autoptr(GError) error = NULL;
  g_autoptr(WyreboxLocalObjectStore) store = NULL;
  g_autoptr(GBytes) first_bytes = g_bytes_new_static(first, sizeof(first) - 1);
  g_autoptr(GBytes) second_bytes = g_bytes_new_static(second, sizeof(second) - 1);
  g_autofree char *first_key = NULL;
  g_autofree char *second_key = NULL;
  ...
  g_assert_true(wyrebox_local_object_store_put_bytes(store, first_bytes, &first_key, &error));
  g_assert_no_error(error);
  g_assert_true(wyrebox_local_object_store_put_bytes(store, second_bytes, &second_key, &error));
  g_assert_no_error(error);
...
test_invalid_key_fails(void)
{
  ...
  output = wyrebox_local_object_store_get_bytes(store, "sha256:not-hex", &error);
...
main(int argc, char **argv)
{
  ...
  g_test_add_func("/object-store/missing-valid-key-fails", test_missing_valid_key_fails);
