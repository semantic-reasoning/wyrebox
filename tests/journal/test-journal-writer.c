...
{
  g_autofree char *root = g_dir_make_tmp("wyrebox-journal-writer-XXXXXX", NULL);
...
test_appends_message_delivered_record(void)
{
  ...
  g_autoptr(GError) error = NULL;
  g_autoptr(WyreboxJournalWriter) writer = NULL;
  g_autoptr(GBytes) payload_bytes = g_bytes_new_static(payload, sizeof(payload));
