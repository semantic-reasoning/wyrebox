    ...
    GError **error)
{
  if (value == NULL || *value == '\0') {
    g_set_error (error,
        ...
        "export request %s is required", field_name);
    return FALSE;
  }
  ...
  for (const char *cursor = value; *cursor != '\0'; cursor++) {
    if (g_ascii_iscntrl (*cursor)) {
      g_set_error (error,
          G_IO_ERROR,
          G_IO_ERROR_INVALID_ARGUMENT,
          "export request %s must not contain control characters", field_name);
      return FALSE;
    }
  }
    ...
    GError **error)
{
  if (after_journal_offset != 0 && after_journal_sequence == 0) {
    g_set_error (error,
        G_IO_ERROR,
        G_IO_ERROR_INVALID_ARGUMENT,
        "export request requires a non-zero afterJournalSequence when "
...
validate_time_range (guint64 start_unix_us, guint64 end_unix_us, GError **error)
{
  if (start_unix_us != 0 && end_unix_us != 0 && start_unix_us > end_unix_us) {
    g_set_error (error,
        G_IO_ERROR,
        G_IO_ERROR_INVALID_ARGUMENT,
        "export request start_unix_us must not be greater than end_unix_us");
    return FALSE;
  }
...
void
wyrebox_daemon_export_request_clear (WyreboxDaemonExportRequest *request)
{
  if (request == NULL)
    return;
...
gboolean
wyrebox_daemon_export_request_init (WyreboxDaemonExportRequest *request,
...
{
  g_auto (WyreboxDaemonExportRequest) next = { 0 };
  ...
  wyrebox_daemon_export_request_clear (request);
  *request = next;
