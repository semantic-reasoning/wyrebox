    ...
    GError **error)
{
  ...
  if (cause == NULL) {
    g_set_error (&fallback,
        G_IO_ERROR,
        G_IO_ERROR_FAILED, "message FETCH request failed without error detail");
    cause = fallback;
  }
...
gboolean
wyrebox_daemon_message_fetch_dispatch (WyreboxDaemonMessageFetchService
    *service, const char *request_id, const char *caller_identity,
    const char *account_identity, const char *tool_identity,
    const char *correlation_id, const WyreboxDaemonMessageFetchRequest *request,
    WyreboxDaemonResponseFrame *out_frame, GError **error)
{
  g_auto (WyreboxDaemonRequestIdentity) identity = { 0 };
  g_autoptr (GError) local_error = NULL;

  g_return_val_if_fail (WYREBOX_IS_DAEMON_MESSAGE_FETCH_SERVICE (service),
      FALSE);
  ...
  if (wyrebox_daemon_message_fetch_service_handle_identity (service,
