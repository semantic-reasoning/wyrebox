...
gboolean
    wyrebox_daemon_wirelog_predicate_query_dispatch
    ...
    WyreboxDaemonResponseFrame * out_frame, GError ** error)
{
  ...
  if (wyrebox_daemon_wirelog_predicate_query_service_handle_identity (service,
          &identity, request, out_frame, &local_error))
    return TRUE;

  return init_error_response_from_cause (out_frame, identity.request_id,
      identity.correlation_id, local_error, error);
}
