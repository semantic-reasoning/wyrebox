    ...
    GError **error)
{
  ...
  if (cause == NULL) {
    g_set_error (&fallback,
        G_IO_ERROR,
        G_IO_ERROR_FAILED,
        "mailbox SELECT request failed without error detail");
    cause = fallback;
  }
...
gboolean
wyrebox_daemon_mailbox_select_dispatch (WyreboxDaemonMailboxSelectService
    ...
    WyreboxDaemonResponseFrame *out_frame, GError **error)
{
  ...
  g_return_val_if_fail (WYREBOX_IS_DAEMON_MAILBOX_SELECT_SERVICE (service),
      FALSE);
  ...
  if (wyrebox_daemon_mailbox_select_service_handle_identity (service,
