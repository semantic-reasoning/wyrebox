...
void
wyrebox_daemon_mailbox_select_request_clear (WyreboxDaemonMailboxSelectRequest
    *request)
{
  if (request == NULL)
    return;
...
gboolean
wyrebox_daemon_mailbox_select_request_init (WyreboxDaemonMailboxSelectRequest
    *request, const char *account_identity, const char *mailbox_id,
    const char *mailbox_name, GError **error)
{
  g_auto (WyreboxDaemonMailboxSelectRequest) next = { 0 };
  ...
  if (has_mailbox_id == has_mailbox_name) {
    g_set_error (error,
        G_IO_ERROR,
        G_IO_ERROR_INVALID_ARGUMENT,
        "mailbox SELECT requires exactly one of mailbox_id or mailbox_name");
    return FALSE;
  }
  ...
  wyrebox_daemon_mailbox_select_request_clear (request);
  *request = next;
