...
init_error_response_from_cause (WyreboxDaemonResponseFrame *out_frame,
    ...
    const char *correlation_id, const GError *cause, GError **error)
{
...
gboolean
    wyrebox_daemon_flag_keyword_update_dispatch
    (WyreboxDaemonFlagKeywordUpdateService * service, const char *request_id,
    ...
    WyreboxDaemonResponseFrame * out_frame, GError ** error)
{
  ...
  g_return_val_if_fail (WYREBOX_IS_DAEMON_FLAG_KEYWORD_UPDATE_SERVICE (service),
      FALSE);
  ...
  if (wyrebox_daemon_flag_keyword_update_service_handle_identity (service,
