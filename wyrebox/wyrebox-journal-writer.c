  ...
  while (size > 0) {
    ...
    if (wrote < 0) {
      ...
      g_set_error(error,
                  ...
                  "failed to write journal record: %s",
...
wyrebox_journal_writer_new(const char *journal_root_dir, GError **error)
{
  ...
  if (journal_root_dir == NULL || *journal_root_dir == '\0') {
    g_set_error(error,
                ...
                "journal root directory is required");
    return NULL;
  }
                             ...
                             GError **error)
{
  ...
  if (self->failed) {
    g_set_error(error,
                G_IO_ERROR,
                G_IO_ERROR_FAILED,
                "journal writer is in failed state");
    return FALSE;
  }
  ...
  if (!validate_event_type(event_type)) {
    g_set_error(error,
                G_IO_ERROR,
                G_IO_ERROR_INVALID_ARGUMENT,
                "invalid journal event type: %d",
                (int) event_type);
    return FALSE;
  }

  event_type_name = g_strdup(wyrebox_journal_event_type_to_string(event_type));
  if (payload != NULL)
  ...
  *out_sequence = 0;
  ...
  g_checksum_update(checksum,
                   (const guchar *) event_type_name,
                   (gssize) strlen(event_type_name));
  ...
  if (!write_all(self->fd,
                 (const guint8 *) event_type_name,
                 strlen(event_type_name),
                 error)) {
    self->failed = TRUE;
    return FALSE;
  }
