...
#define WYREBOX_DAEMON_AUDIT_PAYLOAD_MAGIC "WYREDAU1"
#define WYREBOX_DAEMON_AUDIT_PAYLOAD_MAGIC_LEN 8
#define WYREBOX_DAEMON_AUDIT_PAYLOAD_HEADER_SIZE 34
    ...
    GIOErrorEnum code, GError **error)
{
  ...
  if (payload->final_journal_sequence == 0) {
    g_set_error (error,
        G_IO_ERROR,
    ...
    GError **error)
{
  ...
  guint8 *cursor = NULL;
  gsize payload_len = WYREBOX_DAEMON_AUDIT_PAYLOAD_HEADER_SIZE;

  g_return_val_if_fail (error == NULL || *error == NULL, NULL);

  if (!validate_payload (payload, G_IO_ERROR_INVALID_ARGUMENT, error) ||
      !checked_add_string (&payload_len, payload->request_id, error) ||
  ...
  memcpy (data, WYREBOX_DAEMON_AUDIT_PAYLOAD_MAGIC,
    ...
    WyreboxDaemonAuditPayload *out_payload, GError **error)
{
  ...
  gsize size = 0;
  gsize offset = WYREBOX_DAEMON_AUDIT_PAYLOAD_HEADER_SIZE;
  ...
  if (size < WYREBOX_DAEMON_AUDIT_PAYLOAD_HEADER_SIZE) {
    g_set_error (error,
        G_IO_ERROR,
        G_IO_ERROR_INVALID_DATA, "DaemonAuditRecorded payload is truncated");
    return FALSE;
  ...
  if (memcmp (data, WYREBOX_DAEMON_AUDIT_PAYLOAD_MAGIC,
